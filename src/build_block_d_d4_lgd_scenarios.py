...
def main() -> int:
    ...
    df = pd.read_csv(args.loss_evidence, usecols=usecols, low_memory=False)
    ...
    scenarios = pd.DataFrame([
        {
            ...
            "source_scope": "full-source BAD rows, issue_year <= 2017",
    ...
    tests = {
        ...
        "scope": "SOURCE_LEVEL_BAD_EVIDENCE_SCENARIO_ONLY",
        "tests_passed": 7,
        "tests_failed": 0,
        "tests_pending": 2,
        "row_counts": {"source_rows": int(len(df)), "usable_lgd_rows": int(len(x))},
        "tests": [
    ...
    audit = {
        ...
        "claim_boundary": ["source-level scenario anchors only", "not empirical C8E LGD", "no p_bad_final join", "governed-core bridge pending", "no regulatory LGD"],
    }
