...
def main():
    ...
    try:
        ...
        summary=c.execute("SELECT COUNT(*) FILTER (WHERE materiality_flag), SUM(bad_associated_share) FILTER (WHERE materiality_flag), MAX(materiality_rank) FROM analytics.risk_concentration").fetchone()
        (a.output_dir/'b8_summary.json').write_text(json.dumps({'stage':'B8','status':'BUILT','material_segments':summary[0],'material_bad_associated_share_sum':summary[1],'max_materiality_rank':summary[2],'materiality_rule':'relative_bad_rate > 1.0 AND account_share >= 0.001 (0.1%)','concentration_index_definition':'relative_bad_rate × loan_amount_share; descriptive only'},indent=2)+'\n',encoding='utf-8')
        (a.output_dir/'b8_run_manifest.json').write_text(json.dumps({'stage':'B8','status':'BUILT','source_table':'analytics.segment_risk','ranking_key':'bad_associated_share DESC','single_variable_only':True,'no_combinatorial_fishing':True},indent=2)+'\n',encoding='utf-8')
